% w2_skewed_W.m - W-modified smoother on the same skewed mesh as
% w2_skewed.m. W_K is built from the (straight-sided) reference element
% corners via compute_WK_simplex with an ideal equilateral triangle.
% Expectation: reference and initial figures match w2_skewed exactly;
% smoothed output should look closer to the original (unskewed) w2.m.
...
% Same single vertex move as w2_skewed.m -- one sliver cluster.
moves = [
    0.8, 0.8,  0.72, 0.79;
];
...
% Build nodal W_h from the skewed reference corners (ideal: equilateral).
