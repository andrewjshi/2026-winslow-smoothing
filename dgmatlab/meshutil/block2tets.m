...
switch dim
    ...
    case 3
        tets_per_el = 5;
        if abs(ind)~=1, tets_per_el = 6; end
        nx = size(X,2); ny = size(X,3); nz = size(X,4);
        XT = zeros(3,4,tets_per_el*(nx-1)*(ny-1)*(nz-1));
        in = ind;
        ik = 1;
        for k=1:nz-1
            iz = in;
            for j=1:ny-1
                ...
                for i=1:nx-1
                    ...
                    XT(1:3,1:4,ik:ik+tets_per_el-1) = XL;
                    ik = ik+tets_per_el;
                    in = -in;
                end
                in = -iy;
...
elseif ind == 0 % 6 tets
    XL(:,1,1)= P4; XL(:,2,1)= P5; XL(:,3,1)= P2; XL(:,4,1)= P6;
    XL(:,1,2)= P4; XL(:,2,2)= P5; XL(:,3,2)= P1; XL(:,4,2)= P2;
    XL(:,1,3)= P4; XL(:,2,3)= P0; XL(:,3,3)= P2; XL(:,4,3)= P1;
    XL(:,1,4)= P5; XL(:,2,4)= P7; XL(:,3,4)= P3; XL(:,4,4)= P6;
    XL(:,1,5)= P5; XL(:,2,5)= P6; XL(:,3,5)= P3; XL(:,4,5)= P2;  
    XL(:,1,6)= P5; XL(:,2,6)= P1; XL(:,3,6)= P2; XL(:,4,6)= P3;  
elseif abs(ind) == 2 % 6 tets, four sides flipped
    XL(:,1,1)= P0; XL(:,2,1)= P1; XL(:,3,1)= P2; XL(:,4,1)= P6;
    XL(:,1,2)= P0; XL(:,2,2)= P1; XL(:,3,2)= P6; XL(:,4,2)= P5;
    XL(:,1,3)= P0; XL(:,2,3)= P5; XL(:,3,3)= P6; XL(:,4,3)= P4;
    XL(:,1,4)= P1; XL(:,2,4)= P2; XL(:,3,4)= P6; XL(:,4,4)= P7;
    XL(:,1,5)= P1; XL(:,2,5)= P6; XL(:,3,5)= P5; XL(:,4,5)= P7;  
    XL(:,1,6)= P1; XL(:,2,6)= P3; XL(:,3,6)= P2; XL(:,4,6)= P7;  
end
