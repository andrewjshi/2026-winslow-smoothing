...
if nargin<4 | isempty(bcol), bcol=[.8,1,.8]; end
...
switch dim
 ...
 case 3
  if nargin<4 | isempty(bcol), bcol=[.8,1,.8]; end
  if nargin<5 | isempty(icol), icol=[.6,.8,1]; end
...
if nodes==1
  if dim==2
    ...
    line(p(:,1),p(:,2),p(:,3),'linest','none','marker','.','col',icol,'markers',24);
  end
elseif nodes==2
  for ip=1:size(p,1)
    ...
    if dim==2
      ...
      text(p(ip,1),p(ip,2),p(ip,3),num2str(ip),txtpars{:});
    end
  end
end
if tris==2
