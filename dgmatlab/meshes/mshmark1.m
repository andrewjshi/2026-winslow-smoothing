function msh=mshnaca1(angle,hs,box)

if nargin<1, angle=0; end
if nargin<2, hs=[0.05,0.2,0.2,0.4,0.8,2]; end
if nargin<3, box=[-1,-2; 4,2]; end
...
pv2=[box(1,1),box(1,2); box(2,1),box(1,2); 
     box(2,1),box(2,2); box(1,1),box(2,2)];

[p,t]=polymesh({pv1,pv2},[1,1],[0,1;1,0],[hs(end),1.5],@href,hs);

fdbox=@(p) ddiff(drectangle(p,box(1,1),box(2,1),box(1,2),box(2,2)),dnaca(p,a,theta));
for ii=1:5
  p=bndproj(p,t,fdbox);
end
