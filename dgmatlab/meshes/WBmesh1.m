...
%   nl3 = 3;   % Number of horizontal points x-direction aft-body region
    nlay = 3;  % Number of viscous layers + 1
    nh1 = 7;   % Number of horizontal points y-direction in inner region
    nh2 = 3;   % Number of horizontal points y-direction in inner region
    nv1 = 5;   % Number of vertical points z-direction in lower region
    nv2 = 9;   % Number of vertical points z-direction in middle region
    nv3 = 5;   % Number of vertical points z-direction in upper region
    nl1 = 4;   % Number of horizontal points x-direction pre-body region
    nl2 = 9;  % Number of horizontal points x-direction body region
    nl3 = 6;   % Number of horizontal points x-direction aft-body region
end
...
X1  = block( [nh1, nlay, nv2], [-2,-8,-2,-8, 0.5,0.5,0.5,0.5, -2,-2,-8,-8], points(hex(1,:),:));
X2  = block( [nlay, nl2, nv2], [2,2,2,2, -8,-2,-8,-2, -8,-2,-8,-2], points(hex(2,:),:));
X3  = block( [nh1, nlay, nv2], [0.125,-2,0.125,-2, 2,2,2,2, -8,-8,-2,-2], points(hex(3,:),:));
X4  = block( [nh1, nl2, nlay], [-8,0.125,-2,-2, -8,-8,-2,-2, 2,2,2,2], points(hex(4,:),:)); 
X5  = block( [nh1, nl2, nlay], [-2,-2,-8,0.125, -2,-2,-8,-8, 0.5,0.5,0.5,0.5], points(hex(5,:),:));
...
if (sum(msh.t2t(:) == -1) ~= symmetry),
symmetry
sum(msh.t2t(:) == -1) 
 error('Parity error 1');  end
if (sum(msh.t2t(:) == -2) ~= wall), 
wall
sum(msh.t2t(:) == -2) 
 error('Parity error 2');  end
if (sum(msh.t2t(:) == -3) ~= farfield),
farfield
sum(msh.t2t(:) == -3) 
 error('Parity error 3');  end
