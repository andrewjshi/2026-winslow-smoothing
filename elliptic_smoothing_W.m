...
for it=1:nt
  ...
  if isfield(msh,'W_h') && ~isempty(msh.W_h)
    w11=phi'*msh.W_h(:,1,1,it); w12=phi'*msh.W_h(:,1,2,it);
    w21=phi'*msh.W_h(:,2,1,it); w22=phi'*msh.W_h(:,2,2,it);
    dW2=(w11.*w22-w12.*w21).^2;
    a=(w11.*w11.*G-2*w11.*w12.*F+w12.*w12.*E)./dW2;
    c=(w21.*w21.*G-2*w21.*w22.*F+w22.*w22.*E)./dW2;
    b=-(w11.*w21.*G-(w11.*w22+w12.*w21).*F+w12.*w22.*E)./dW2;
  else
    a=G; b=F; c=E;
  end
...
for it=1:nt
  ...
  if isfield(msh,'W_h') && ~isempty(msh.W_h)
    ...
    w21=phi'*msh.W_h(:,2,1,it); w22=phi'*msh.W_h(:,2,2,it);
    dW2=(w11.*w22-w12.*w21).^2;
    g11=(w11.*w11.*G-2*w11.*w12.*F+w12.*w12.*E)./dW2;
    g22=(w21.*w21.*G-2*w21.*w22.*F+w22.*w22.*E)./dW2;
    g12=(w11.*w21.*G-(w11.*w22+w12.*w21).*F+w12.*w22.*E)./dW2;
    g21=g12;
  else
    g11=G;
    g22=E;
